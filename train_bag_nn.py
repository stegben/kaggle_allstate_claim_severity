...
def main():
    ...
    print('===== train model')
    model = BagDNN(base_model_folder='../allstate_basemodel_new/.base_models/')
    model.fit(train_x, train_y, n_folds=10, random_state=12354)

    validation_pred = model.predict(validation_x)
    mae = mean_absolute_error(validation_y, validation_pred)
    print(mae)
    model_fname = '../allstate_basemodel/%4.0f_dnn_with_' % mae
